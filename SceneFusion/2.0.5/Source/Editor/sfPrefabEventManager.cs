    ...
    public class sfPrefabEventManager
    {
        ...
        private void TryAddCreateEvent(GameObject gameObject)
        {
            ...
            CreateGameObjectHierarchyEventArgs data = 
                new CreateGameObjectHierarchyEventArgs(sfUnityUtils.GetUnityId(prefab), prefab.scene);
            m_eventBuilder.PushCreateGameObjectHierarchyEvent(ref data);
            m_stageMapDirty = true;
        }

        /// <summary>
        /// Adds a delete event if the <paramref name="uobjectId"/> is for a stage object in the
        /// <see cref="sfPrefabStageMap"/>, with the id changed to the id of the prefab object.
        /// </summary>
        /// <param name="uobjectId">Instance id of deleted object</param>
        private void TryAddDeleteEvent(ulong uobjectId)
        {
            GameObject prefab = m_prefabStageMap.GetPrefabObject<GameObject>(uobjectId);
            if (prefab == null)
            {
                return;
            }
            DestroyGameObjectHierarchyEventArgs data =
                new DestroyGameObjectHierarchyEventArgs(sfUnityUtils.GetUnityId(prefab), prefab.scene);
        ...
        private void TryAddPropertiesChangeEvent(UObject uobj)
        {
            ...
            ChangeGameObjectOrComponentPropertiesEventArgs data = 
                new ChangeGameObjectOrComponentPropertiesEventArgs(sfUnityUtils.GetUnityId(prefab), new Scene());
        ...
        private void TryAddParentChangeEvent(GameObject gameObject)
        {
            ...
            ChangeGameObjectParentEventArgs data =
                new ChangeGameObjectParentEventArgs(sfUnityUtils.GetUnityId(prefab), prefab.scene, sfUnityUtils.NO_ID,
                prefab.scene, sfUnityUtils.NO_ID);
        ...
        private void TryAddChildrenOrderChangeEvent(GameObject gameObject)
        {
            ...
            ChangeGameObjectParentEventArgs data = new ChangeGameObjectParentEventArgs(sfUnityUtils.NO_ID,
                prefab.scene, sfUnityUtils.GetUnityId(prefab), prefab.scene, sfUnityUtils.GetUnityId(prefab));
            m_eventBuilder.PushChangeGameObjectParentEvent(ref data);
        }
        ...
        private void TryAddStructureChangeEvent(GameObject gameObject)
        {
            ...
            ChangeGameObjectStructureEventArgs data =
                new ChangeGameObjectStructureEventArgs(sfUnityUtils.GetUnityId(prefab), prefab.scene);
        ...
        private void TryAddHierarchyStructureChangeEvent(GameObject gameObject)
        {
            ...
            ChangeGameObjectStructureHierarchyEventArgs data =
                new ChangeGameObjectStructureHierarchyEventArgs(sfUnityUtils.GetUnityId(prefab), prefab.scene);
        ...
        private void TryAddPropertyModifications(UndoPropertyModification[] modifications)
        {
            ...
            for (int i = 0; i < modifications.Length; i++)
            {
                ...
                else
                {
                    ...
                    TryAddModifiedPrefab(sfUnityUtils.GetUObjectId(modification.currentValue.target));
                }
            }
        ...
        /// <summary>
        /// If the <paramref name="uobjectId"/> is for a prefab, adds the prefab to the set of prefabs with known 
        ...
        private void TryAddModifiedPrefab(ulong uobjectId)
        {
            string path = sfUnityUtils.GetAssetPath(uobjectId);

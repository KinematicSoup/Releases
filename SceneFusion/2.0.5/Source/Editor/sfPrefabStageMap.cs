...
namespace KS.SceneFusion.Client.Unity.Editor
{
    ...
    public class sfPrefabStageMap
    {
        ...
        private Dictionary<ulong, UObject> m_stageToPrefabMap = new Dictionary<ulong, UObject>();
        ...
        {
            return stageObject == null ? null : GetPrefabObject<T>(sfUnityUtils.GetUObjectId(stageObject));
        }

        /// <summary>Gets the prefab object for a stage object.</summary>
        /// <param name="uobjectId">Id of stage object to get prefab object for.</param>
        /// <returns>The prefab object for the stage object, or null if none was found.</returns>
        public T GetPrefabObject<T>(ulong uobjectId) where T : UObject
        {
            UObject prefab;
            m_stageToPrefabMap.TryGetValue(uobjectId, out prefab);
            return prefab as T;
        }
        ...
        private void AddMapping(GameObject stageObject, GameObject prefabObject)
        {
            ...
            m_stageToPrefabMap[sfUnityUtils.GetUObjectId(stageObject)] = prefabObject;
            ...
            for (int i = 0; i < stageComponents.Length; i++)
            {
                ...
                {
                    m_stageToPrefabMap[sfUnityUtils.GetUObjectId(stageComponents[i])] = prefabComponents[i];
                }
            }

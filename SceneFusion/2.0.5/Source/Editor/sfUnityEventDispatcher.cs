...
namespace KS.SceneFusion.Client.Unity.Editor
{
    ...
    public class sfUnityEventDispatcher
    {
        ...
        /// <param name="uobjectId">Id of game object that was deleted.</param>
        public delegate void DeleteCallback(ulong uobjectId);
        ...
        /// <param name="uobjectId">Id of modified object.</param>
        public delegate void ModifyObjectHandler(ulong uobjectId);
        ...
        // prevent double invoking events. Keys are instance/entity ids.
        private Dictionary<ulong, Events> m_invokedEventMap = 
            new Dictionary<ulong, Events>();
        ...
        private void InvokeCreateEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            id = EntityId.ToULong(data.entityId);
#else
            unchecked { id = (ulong)data.instanceId; }
#endif
            SetEventFlag(id, Events.CREATE);
            if (OnCreate == null)
            {
                return;
            }
            GameObject gameObj = sfUnityUtils.GetUObject<GameObject>(id);
            if (gameObj != null)
            {
                SetEventFlag(id, Events.CREATE);
                OnCreate(gameObj);
        ...
        private void InvokeCreatePrefabChildEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            unchecked { id = (ulong)data.instanceId; }
#endif
            if (!SetEventFlag(id, Events.CREATE_CHILD) || OnCreatePrefabChild == null)
            {
                return;
            }
            GameObject gameObj = sfUnityUtils.GetUObject<GameObject>(id);
        ...
        /// <summary>Invokes the on delete event.</summary>
        /// <param name="uobjectId">Id of game object that was deleted.</param>
        public void InvokeOnDelete(ulong uobjectId)
        {
            if (OnDelete != null)
            {
                OnDelete(uobjectId);
        ...
        private void InvokeHierarchyStructureChangeEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            unchecked { id = (ulong)data.instanceId; }
#endif
            if (!SetEventFlag(id, Events.CHANGE_HIERARCHY_STRUCTURE) ||
        ...
        private void InvokeUpdatePrefabInstanceEvents(ref ObjectChangeEventStream stream, int index)
        {
            ...
            ulong id;
#if UNITY_6000_4_OR_NEWER
            foreach (EntityId entityId in data.entityIds)
            {
                id = EntityId.ToULong(entityId);
#else
            foreach (int instanceId in data.instanceIds)
            {
                unchecked { id = (ulong)instanceId; }
#endif
        ...
        private void InvokeParentChangeEvent(ref ObjectChangeEventStream stream, int index)
        {
...
#if UNITY_6000_4_OR_NEWER
            id = EntityId.ToULong(data.entityId);
            oldParentId = EntityId.ToULong(data.previousParentEntityId);
            newParentId = EntityId.ToULong(data.newParentEntityId);
#else
            unchecked 
            { 
                id = (ulong)data.instanceId;
                oldParentId = (ulong)data.previousParentInstanceId;
                newParentId = (ulong)data.newParentInstanceId;
            }
#endif
            ...
            GameObject gameObj;
            if (id == 0 && oldParentId == newParentId)
            {
                if (!SetEventFlag(newParentId, Events.REORDER_CHILDREN) || OnReorderChildren == null)
                {
                    return;
                }
                gameObj = sfUnityUtils.GetUObject<GameObject>(newParentId);
                if (gameObj != null)
                {
            ...
            if (!SetEventFlag(id, Events.CHANGE_PARENT) || OnParentChange == null)
            {
                return;
            }
            gameObj = sfUnityUtils.GetUObject<GameObject>(id);
            if (gameObj != null)
            {
        ...
        private void InvokeReorderChildrenEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            unchecked { id = (ulong)data.instanceId; }
#endif
            if (!SetEventFlag(id, Events.REORDER_CHILDREN) || OnReorderChildren == null)
            {
                return;
            }
            GameObject gameObj = sfUnityUtils.GetUObject<GameObject>(id);
            ...
            bool invokeAddRemoveComponentsForPrefabVariant)
        {
            ...
            unchecked { id = (ulong)data.instanceId; }
#endif
            bool canInvoke = SetEventFlag(id, Events.CHANGE_PROPERTIES);
            if (sfUndoManager.Get().HasPendingPropertyModifications)
            {
                return;
            }
            UObject uobj = sfUnityUtils.GetUObject(id);
            if (uobj != null)
            {
                ...
                if (invokeAddRemoveComponentsForPrefabVariant)
                {
                    ...
                    if (gameObj != null && PrefabUtility.IsPartOfPrefabAsset(gameObj) &&
                        ...
                        SetEventFlag(id, Events.ADD_REMOVE_COMPONENT) && OnAddOrRemoveComponents != null)
                    {
        ...
        private void InvokePropertiesChangedEventForAsset(ref ObjectChangeEventStream stream, int index)
        {
            ...
            unchecked { id = (ulong)data.instanceId; }
#endif
            if (!SetEventFlag(id, Events.CHANGE_PROPERTIES) || OnPropertiesChanged == null ||
        ...
        private void InvokeAddOrRemoveComponentsEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            unchecked { id = (ulong)data.instanceId; }
#endif
            if (!SetEventFlag(id, Events.ADD_REMOVE_COMPONENT) || OnAddOrRemoveComponents == null)
            {
                return;
            }
            GameObject gameObject = sfUnityUtils.GetUObject<GameObject>(id);
            if (gameObject != null)
            {
        ...
        /// <param name="uobjectId">Id of modified uobject.</param>
        public void InvokeOnModifyObject(ulong uobjectId)
        {
            if (OnModifyObject != null)
            {
                OnModifyObject(uobjectId);
            }
        }

        /// <summary>Sets event flag(s) in the invoked event map for a uobject.</summary>
        /// <param name="uobjectId">Id of uobject to set event for.</param>
        /// <param name="flags">flags to set.</param>
        /// <returns>false if the flag(s) were already set.</returns>
        private bool SetEventFlag(ulong uobjectId, Events flags)
        {
            Events events;
            if (m_invokedEventMap.TryGetValue(uobjectId, out events) && (events & flags) == flags)
            {
                return false;
            }
            m_invokedEventMap[uobjectId] = events | flags;

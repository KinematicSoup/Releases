...
namespace KS.Reactor.Client.Unity.Editor
{
    ...
    public class ksServerInstaller : ksSingleton<ksServerInstaller>
    {
        ...
        public FileStates Check(bool checkBuildFiles = true, bool checkServerFiles = true, bool attemptDownload = false, bool logWarnings = true)
        {
            ...
            if (checkBuildFiles)
            {
                List<string> buildFiles = new List<string>() { "KSReflectionTool.dll", "KSCommon.dll", "KSReactor.dll", "KSLZMA.dll" };
                if (Application.platform == RuntimePlatform.WindowsEditor)
                {
                    buildFiles.Add("KSReflectionTool.exe");
                }
                else if (Application.platform == RuntimePlatform.OSXEditor)
                {
                    buildFiles.Add("KSReflectionTool");
                }
            ...
            if (checkServerFiles)
            {
                List<string> serverFiles = new List<string> { "Reactor.dll", "KSCommon.dll", "KSReactor.dll", "KSLZMA.dll" };
                if (Application.platform == RuntimePlatform.WindowsEditor)
                {
                    serverFiles.Add("Reactor.exe");
                    serverFiles.Add("KSPhysX.dll");
                    serverFiles.Add("ServerModule.dll");
                }
                else if (Application.platform == RuntimePlatform.OSXEditor)
                {
                    serverFiles.Add("Reactor");
                    serverFiles.Add("KSPhysX.dylib");
                    serverFiles.Add("ServerModule.dylib");
                }

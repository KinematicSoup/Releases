...
namespace KS.Reactor.Client.Unity.Editor
{
    ...
    public class ksSerializableMultiTypeDrawer : PropertyDrawer
    {
        ...
        /// <summary>Draws an editor for a <see cref="Vector2Int"/>.</summary>
        /// <param name="position">Position to draw at.</param>
        /// <param name="label">Label for the value.</param>
        /// <param name="value">Value</param>
        /// <returns>New value</returns>
        private Vector2Int DrawVector2Int(Rect position, GUIContent label, Vector2Int value)
        {
            // If you pass a non-empty label to Unity's Vector2IntField, it sometimes takes up 2 lines instead of 1
            // depending on how much width is available, which messes up our positioning. To force it to use 1 line,
            // we add a separate prefix label, then call Vector2IntField with an empty label.
            Rect labelPosition = position;
            position = EditorGUI.PrefixLabel(labelPosition, label);
            float dx = (position.x - labelPosition.x) / 3f;
            position.x -= dx;
        ...
        /// <summary>Draws an editor for a <see cref="Vector3Int"/>.</summary>
        /// <param name="position">Position to draw at.</param>
        /// <param name="label">Label for the value.</param>
        /// <param name="value">Value</param>
        /// <returns>New value</returns>
        private Vector3Int DrawVector3Int(Rect position, GUIContent label, Vector3Int value)
        {
            // If you pass a non-empty label to Unity's Vector3IntField, it sometimes takes up 2 lines instead of 1
            // depending on how much width is available, which messes up our positioning. To force it to use 1 line,
            // we add a separate prefix label, then call Vector3IntField with an empty label.
            Rect labelPosition = position;
            position = EditorGUI.PrefixLabel(labelPosition, label);
            float dx = (position.x - labelPosition.x) / 3f;
            position.x -= dx;
            ...
            ref bool expanded)
        {
            switch (multiType.Type)
            {
                ...
                case ksMultiType.Types.VECTOR2_INT: multiType.Vector2Int = DrawVector2Int(position, label, multiType); break;
                case ksMultiType.Types.VECTOR3_INT: multiType.Vector3Int = DrawVector3Int(position, label, multiType); break;
                ...
                case ksMultiType.Types.VECTOR2_INT_ARRAY: multiType.UnityVector2IntArray = DrawArray<Vector2Int>(position, typeWidth, label, multiType, DrawVector2Int, ref expanded); break;
                case ksMultiType.Types.VECTOR3_INT_ARRAY: multiType.UnityVector3IntArray = DrawArray<Vector3Int>(position, typeWidth, label, multiType, DrawVector3Int, ref expanded); break;
        ...
        private ksMultiType MultiTypeAs(ref ksMultiType multiType, ksMultiType.Types type)
        {
            ...
            switch (type)
            {
                ...
                case ksMultiType.Types.VECTOR2_INT_ARRAY: return new ksVector2Int[0];
                case ksMultiType.Types.VECTOR3_INT_ARRAY: return new ksVector3Int[0];

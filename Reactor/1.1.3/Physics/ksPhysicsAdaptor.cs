...
namespace KS.Reactor.Client.Unity
{
    ...
    public class ksPhysicsAdaptor : ksBasePhysics
    {
            ...
            out ksICollider collider)
        {
            ...
            ksQueryHitTypes result = args.Filter.Filter(queryCollider, collider, resultType, args);

            // Convert blocking hits to touches if the NO_BLOCK flag is set.
            if (result == ksQueryHitTypes.BLOCK && (args.Flags & ksQueryFlags.NO_BLOCK) != 0)
            {
                return ksQueryHitTypes.TOUCH;
            }
            return result;

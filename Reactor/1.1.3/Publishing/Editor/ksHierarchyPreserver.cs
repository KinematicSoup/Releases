    ...
    public class ksHierarchyPreserver
    {
        ...
        private struct HierarchyWindowState
        {
            ...
            public HierarchyWindowState(ksReflectionObject sceneHierarchyRO)
            {
                ...
                EntityId[] entityIds = new EntityId[expandedObjects.Count];
                for (int i = 0; i < expandedObjects.Count; i++)
                {
                    entityIds[i] = expandedObjects[i].GetEntityId();
                }
#else
                int[] entityIds = new int[expandedObjects.Count];
                for (int i = 0; i < expandedObjects.Count; i++)
                {
                    entityIds[i] = expandedObjects[i].GetInstanceID();
                }
#endif
                ExpandedIds = new GlobalObjectId[expandedObjects.Count];
                GlobalObjectId.GetGlobalObjectIdsSlow(entityIds, ExpandedIds);
            }
...
#if UNITY_6000_3_OR_NEWER
                    EntityId[] entityIds = new EntityId[ExpandedIds.Length];
                    GlobalObjectId.GlobalObjectIdentifiersToEntityIdsSlow(ExpandedIds, entityIds);
#else
                    int[] entityIds = new int[ExpandedIds.Length];
                    GlobalObjectId.GlobalObjectIdentifiersToInstanceIDsSlow(ExpandedIds, entityIds);
#endif

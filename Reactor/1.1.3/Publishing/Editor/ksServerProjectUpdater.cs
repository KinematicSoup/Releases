...
namespace KS.Reactor.Client.Unity.Editor
{
    ...
    public class ksServerProjectUpdater 
    {
                ...
                foreach (XmlNode node in nodeList)
                {
                    switch (node.Name)
                    {
                        ...
                        case "HintPath":
                            ...
                            break;
                        case "Exec":
                            node.Attributes["Command"].Value = GetProxyCommand(outputPath, proxyPath);
                            break;
                    }
                }
                SaveProject(projectXml);
        ...
        public void AddFileToProject(string path)
        {
            ...
            path = ksPathUtils.MakeRelative(path, ksPaths.ServerRuntimeProjectDir);
        ...
        private HashSet<string> GetCommonAndServerFiles(string ext)
        {
            ...
            foreach (string path in ksPaths.CommonFolders)
            {
                GetFilesInAsmDefFolder(path, ext, files, ksPaths.ServerRuntimeProjectDir);
            }
            foreach (string path in ksPaths.ServerFolders)
            {
                GetFilesInAsmDefFolder(path, ext, files, ksPaths.ServerRuntimeProjectDir);
        ...
        /// <summary>Gets output and proxy paths relative to the ksServerRuntime project directory..</summary>
        ...
        private void GetRelativePaths(out string outputPath, out string proxyPath, out string intermediatePath)
        {
            ...
            Uri localServerPathUri = null;
            try
            {
                localServerPathUri = new Uri(ksPaths.ServerDir);
            }
            catch (UriFormatException)
            {
                localServerPathUri = new Uri(root, ksPaths.ServerDir);
            }

            Uri serverRuntimeUri = new Uri(root, ksPaths.ServerRuntimeProjectDir);
            outputPath = serverRuntimeUri.MakeRelativeUri(localServerPathUri).ToString();
            intermediatePath = serverRuntimeUri.MakeRelativeUri(new Uri(root, ksPaths.External + "intermediate/")).ToString();
            proxyPath = serverRuntimeUri.MakeRelativeUri(new Uri(root, ksPaths.Proxies)).ToString();
        }

        /// <summary>Gets the command to generate proxy scripts.</summary>
        /// <param name="outputPath">Path to the local server.</param>
        /// <param name="proxyPath">Path to put proxy scripts in.</param>
        /// <param name="useXMLAttributeQuotes">
        ///    Set to true when writing xml attributes directly in a file to use "&quot;" for escape quotes around paths.
        /// </param>
        /// <returns>Command to generate proxy scripts.</returns>
        private string GetProxyCommand(string outputPath, string proxyPath, bool useXMLAttributeQuotes = false)
        {
            string escapeQuotes = useXMLAttributeQuotes ? "&quot;" : "\"";
            return $"{escapeQuotes}{outputPath}KSServerRuntime/KSReflectionTool{escapeQuotes}" +
                $" {escapeQuotes}{proxyPath}{escapeQuotes}";
        }
...
<Project Sdk=""Microsoft.NET.Sdk"">
  ...
  <PropertyGroup Condition=""'$(Configuration)' == 'LocalDebug'"">
    <AssemblyName>KSServerRuntime.Local</AssemblyName>
    <DefineConstants>REACTOR_SERVER;REACTOR_LOCAL_SERVER;TRACE;DEBUG;</DefineConstants>
    <Optimize>false</Optimize>
  </PropertyGroup>
  <PropertyGroup Condition=""'$(Configuration)' == 'OnlineRelease'"">
    <AssemblyName>KSServerRuntime</AssemblyName>
    <DefineConstants>REACTOR_SERVER;TRACE;</DefineConstants>
    <Optimize>true</Optimize>
  </PropertyGroup>
  <PropertyGroup Condition=""'$(Configuration)' == 'OnlineDebug'"">
    <AssemblyName>KSServerRuntime</AssemblyName>
    <DefineConstants>REACTOR_SERVER;TRACE;DEBUG;</DefineConstants>
    <Optimize>false</Optimize>
  </PropertyGroup>
  ...
  <Target Name=""PostBuild"" AfterTargets=""PostBuildEvent"" Condition=""'$(Configuration)' == 'LocalRelease' or '$(Configuration)' == 'LocalDebug'"">
    <Exec Command=""" + GetProxyCommand(outputPath, proxyPath, true) + @""" />
  </Target>
</Project>
";
            ksPathUtils.Create(ksPaths.ServerRuntimeProjectDir, true);

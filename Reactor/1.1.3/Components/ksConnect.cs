...
namespace KS.Reactor.Client.Unity
{
    ...
    public class ksConnect : MonoBehaviour
    {
        ...
        [Tooltip("Use a secure web socket connection protocol for connecting to localhost in webgl builds using the LOCAL connect mode.")]
        public bool SecureLocalWebSockets = false;

        [Tooltip("Event handlers called when after getting the list of running rooms. For REMOTE and LOCAL connections this is called " +
            "with a single room. If there are no handlers, it will automatically connect to the first room. If there are any handlers, " +
            "it will not connect automatically and the handlers are responsible for making a connection.")]

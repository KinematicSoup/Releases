            ...
            if (spColliderData != null)
            {
                ...
                if (m_foldout)
                {
                    ...
                    while (spColliderData.NextVisible(enterChildren))
                    {
                        ...
                        else if (spColliderData.name == nameof(ksColliderData.Existence))
                        {
                            EditorGUI.BeginChangeCheck();
                            EditorGUILayout.PropertyField(spColliderData);
                            if (EditorGUI.EndChangeCheck())
                            {
                                // Mark the target colliders dirty so they can be rebaked, allowing the
                                // PreBakeColliderSystem to remove server-only colliders, and causing formerly server-
                                // only colliders to have their colliders baked.
